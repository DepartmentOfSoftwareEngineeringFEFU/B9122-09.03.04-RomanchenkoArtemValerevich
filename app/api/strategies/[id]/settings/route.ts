...
import { serializeStrategySettings } from "@/lib/server/serializers"
import { normalizePctInput } from "@/lib/server/strategy-config"
...
export async function GET(
  ...
  if (!setting) return NextResponse.json({ success: false, error: "Настройки не найдены" }, { status: 404 })

  return NextResponse.json({ success: true, data: serializeStrategySettings(setting) })
}
...
export async function PUT(
  ...
  const updated = await prisma.strategySetting.update({
    where: { id: setting.id },
    data: settingsUpdateData(body),
  })

  return NextResponse.json({ success: true, data: serializeStrategySettings(updated) })
}
