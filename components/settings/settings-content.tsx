...
export function SettingsContent() {
  ...
  const handleTestConnection = async () => {
    if (!validate()) return
    setIsChecking(true)
    await checkConnection(apiKey, secretKey, passphrase)
    setIsChecking(false)
  }
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true)
    try {
      const res = await fetch("/api/okx/credentials", {
        method: "PUT",
      ...
      if (res.ok) {
        ...
        // Mask after save
        setApiKey(apiKey.slice(0, 4) + "****" + apiKey.slice(-4))
        setSecretKey("")
        setPassphrase("")
      } else {
        toast({ title: "Ошибка", description: "Не удалось сохранить учётные данные.", variant: "destructive" })
      }
    } catch {
  ...
  const handleDelete = async () => {
    try {
      await fetch("/api/okx/credentials", { method: "DELETE" })
      setApiKey("")
      setSecretKey("")
      setPassphrase("")
      setErrors({})
      ...
      toast({ title: "Ошибка", description: "Не удалось удалить учётные данные.", variant: "destructive" })
    }
  }
  ...
  return (
    <Card>
      ...
      <CardFooter className="border-t border-border px-6 py-4 flex flex-col sm:flex-row gap-3 sm:justify-between">
        <Button variant="destructive" type="button" onClick={handleDelete}>Удалить</Button>
